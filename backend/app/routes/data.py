...
def _log(db, user, action: str, entity_type: str, entity_id: str, details: str):
    """Quick activity log helper with user attribution."""
    try:
...
async def create_company(data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    data["id"] = uuid4()
...
async def remove_company(company_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    db_svc.delete_company(db, company_id)
    return {"success": True}


@router.delete("/companies")
async def remove_all_companies(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def export_companies_csv(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.get("/leads")
async def list_leads(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    leads = db_svc.load_leads(db)
    return {"data": [db_svc.lead_db_to_response(l) for l in leads]}


@router.get("/leads/export")
async def export_leads_csv(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_lead(lead_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.post("/leads")
async def create_lead(data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    data["id"] = uuid4()
...
async def update_lead(lead_id: UUID, data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    data["id"] = lead_id
    obj = db_svc.save_lead(db, data)
    return {"success": True, "data": db_svc.lead_db_to_response(obj)}
...
async def remove_lead(lead_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    db_svc.delete_lead(db, lead_id)
    return {"success": True}


@router.delete("/leads")
async def remove_all_leads(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
MAX_ENSURE_ROUNDS = 4  # check + regen rounds before giving up


async def _ensure_postable(post_obj, api_key: str, db) -> object:
    """Verify a post and auto-regenerate until it reaches >= 90% score with no
    false claims.  Returns the (possibly updated) post ORM object.

    This is the SINGLE source of truth for the generate-verify-regen loop.
    Both single-generate and weekly-generate call this.
    """
    for attempt in range(MAX_ENSURE_ROUNDS + 1):  # 0 = initial check, 1-N = regen
        ...
        post_obj.verification_status = "verified" if is_postable else verification.get("status", "issues_found")
        post_obj.verification_score = score
        post_obj.verification_json = json.dumps(verification, ensure_ascii=False)
        db.commit()

        if is_postable:
            logger.info(f"[EnsurePostable] Attempt {attempt}: score={score:.0%} — POSTABLE")
            break

        if attempt >= MAX_ENSURE_ROUNDS:
            logger.warning(f"[EnsurePostable] Giving up after {attempt} rounds, score={score:.0%}")
            break

        # ── Extract issues for targeted regeneration ───────────────
        issues = []
        for c in verification.get("claims", []):
            v = c.get("verdict", "").lower()
            if v == "false":
        ...
        if not issues:
            logger.info(f"[EnsurePostable] No actionable issues despite score={score:.0%}, accepting")
            break

        logger.info(f"[EnsurePostable] Attempt {attempt}: score={score:.0%}, {len(issues)} issues — regenerating...")

        # Regenerate (retry up to 2x for meta-response)
        regen_ok = False
        for regen_retry in range(2):
            try:
                ...
                logger.warning(f"[EnsurePostable] Regen retry {regen_retry+1}: {ve}")

        if not regen_ok:
            logger.warning("[EnsurePostable] Regen returned meta-responses, keeping current")
            break
        ...
        db.commit()
        logger.info(f"[EnsurePostable] Regenerated (round {attempt+1}/{MAX_ENSURE_ROUNDS})")
        # Loop continues → next iteration will cross-check the new content
...
async def list_social_posts(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def verify_social_post(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def regenerate_social_post(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    """Regenerate a social post: one explicit regen pass, then _ensure_postable loop."""
    from ..models.db import SocialPostDB
    post = db.query(SocialPostDB).filter(SocialPostDB.id == post_id).first()
    if not post:
        raise HTTPException(404, "Post nicht gefunden.")
    ...
    # Extract issues from current verification for the first regen pass
    issues = []
    try:
    ...
    try:
        # First explicit regeneration pass using known issues
        ...
        timestamp_str = datetime.utcnow().strftime("%d.%m.%Y %H:%M UTC")
        content_with_ts = result["content"]
        if "🔗 www.harpocrates-corp.com" in content_with_ts:
            content_with_ts = content_with_ts.replace(
                "🔗 www.harpocrates-corp.com",
                f"\n\n📅 {timestamp_str}\n\n🔗 www.harpocrates-corp.com",
            )
        else:
            content_with_ts += f"\n\n📅 {timestamp_str}"
        ...
        # Now run _ensure_postable to verify + auto-fix until >= 90%
        post = await _ensure_postable(post, api_key, db)
...
async def remove_social_post(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    db_svc.delete_social_post(db, post_id)
    return {"success": True}


@router.post("/social-posts/{post_id}/mark-copied")
async def mark_post_copied(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def publish_to_linkedin(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_pending_posts(user: dict = Depends(get_current_user_or_apikey), db: Session = Depends(get_db)):
...
async def mark_post_published(post_id: UUID, body: MarkPublishedBody = Body(default=MarkPublishedBody()), user: dict = Depends(get_current_user_or_apikey), db: Session = Depends(get_db)):
...
async def update_social_post_content(post_id: UUID, body: UpdatePostContentBody, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def cancel_publish(post_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def get_content_calendar(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def schedule_post(post_id: UUID, body: dict = Body(...), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def list_address_book(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def create_address_book_entry(data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def add_lead_to_address_book(lead_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def add_leads_to_address_book_batch(data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_address_book_entry(entry_id: UUID, data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    data["id"] = entry_id
    obj = db_svc.save_address_book_entry(db, data)
    return {"success": True, "data": db_svc.address_book_to_response(obj)}
...
async def remove_address_book_entry(entry_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    db_svc.delete_address_book_entry(db, entry_id)
    return {"success": True}


@router.put("/address-book/{entry_id}/status")
async def update_address_book_status(entry_id: UUID, data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def permanently_delete_address_book_entry(entry_id: UUID, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def export_address_book_csv(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def list_blocklist(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def add_to_blocklist(email: str, reason: str = "", user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def remove_from_blocklist(email: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
@router.put("/settings")
async def update_settings(data: dict, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def import_csv(file: UploadFile = File(...), user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def dashboard(user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
